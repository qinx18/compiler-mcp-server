...
class CompilerMCPServer:
    ...
    async def analyze_vectorization(self, code: str, session_id: Optional[str] = None) -> VectorizationAnalysis:
        """
        ...
        dependencies = await self.dependency_analyzer.analyze_loop_carried_dependencies(code)
    ...
    def _determine_specific_status(
        ...
        if any("unsafe" in issue.lower() or "alias" in issue.lower() for issue in issues):
    ...
    async def _compile_with_diagnostics(self, code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            cmd = [self.compiler_path] + self.base_flags + [
            ...
            return {
                "status": status,
                "messages": messages,
                "assembly": result.stdout if status == CompilationStatus.SUCCESS else None,
    ...
    def _determine_compilation_status(self, messages: List[str], returncode: int) -> CompilationStatus:
    ...
    def _parse_vectorization_messages(self, messages: List[str]) -> List[str]:
        ...
        vectorization_keywords = [
            "vectoriz", "dependenc", "alias", "alignment", 
            "unsafe", "cost model", "iteration", "parallel"
        ]
    ...
    async def _generate_suggestions(
        ...
        if any("alignment" in issue.lower() for issue in issues):
            suggestions.append({
                ...
                "confidence": 0.7,
                "directives": ["__attribute__((aligned(32)))", "#pragma GCC aligned"]
            })
        ...
        suggestions.append({
            ...
            "confidence": 0.6,
            "options": ["#pragma GCC ivdep", "#pragma omp simd", "#pragma vector always"]
        })
...
"""
    ...
    def _check_session_patterns(self, session_id: str, dep: DependencyInfo) -> bool:
        ...
        for transform in session.get("successful_transforms", []):
            if (transform.get("dependency_type") == "loop_carried" and 
                transform.get("variable") == dep.variable):
                return True
        
        return False
    
    def _get_previous_success(self, session_id: str, dep: DependencyInfo) -> Dict[str, Any]:
...
class DependencyAnalyzer:
    ...
    async def analyze_loop_carried_dependencies(self, code: str) -> List[DependencyInfo]:
        """
...
# Register MCP tools
@mcp.tool()
async def analyze_vectorization_failure(code: str, session_id: Optional[str] = None) -> str:
    """
    ...
    response = f"Vectorization Analysis for provided code:\n\n"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mode', choices=['stdio', 'http', 'test'], default='stdio',
