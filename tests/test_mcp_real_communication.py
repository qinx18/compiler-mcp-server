...
class MCPServerCommunicator:
    ...
    def create_request(self, method: str, params: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    async def test_tools_list_request(self, mcp_server):
        ...
        if response:
            ...
            if "result" in response:
                ...
                expected_tools = ["analyze_vectorization_failure", "create_compilation_session"]
                ...
                print(f"⚠️ Tools list request failed: {response.get('error', 'Unknown error')}")
    ...
    async def test_tool_call_execution(self, mcp_server):
        ...
        if response:
            ...
            if "result" in response:
                ...
                print(f"✅ Tool execution successful, analysis length: {len(content)} chars")
            else:
                error = response.get("error", {})
                print(f"⚠️ Tool execution failed: {error.get('message', 'Unknown error')}")
    ...
    async def test_session_management_tool(self, mcp_server):
        ...
        if response:
            ...
            if "result" in response:
                ...
                error = response.get("error", {})
                print(f"⚠️ Session creation failed: {error.get('message', 'Unknown error')}")
...
class TestMCPProtocolEdgeCases:
    ...
    async def test_invalid_json_handling(self, mcp_server):
        ...
        if mcp_server.server_process:
            ...
            assert mcp_server.server_process.poll() is None, "Server crashed on invalid JSON"
    
    @pytest.mark.asyncio
    async def test_unknown_method_handling(self, mcp_server):
        """Test how server handles unknown methods"""
        unknown_request = mcp_server.create_request("unknown_method", {"param": "value"})
...
class TestMCPPerformance:
    ...
    async def test_large_code_analysis(self, mcp_server):
        ...
        start_time = time.time()
        mcp_server.send_message(tool_call_request)
        response = mcp_server.read_response(timeout=15.0)  # Longer timeout for large analysis
        end_time = time.time()
        ...
        if response:
            ...
            assert processing_time < 10.0, f"Analysis took too long: {processing_time:.2f}s"
